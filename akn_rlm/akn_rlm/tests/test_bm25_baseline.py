"""Tests for the BM25-only baseline pipeline.
...
def _make_pipeline(hits: list[BM25Hit], doc_title: str = "قانون الأسرة"):
    bm25 = MagicMock()
    ...
    return build_bm25_pipeline(bm25=bm25, registry=registry), bm25, registry


def _hit(doc_id: str, ref: str, text: str, score: float = 1.0) -> BM25Hit:
    return BM25Hit(
...
def test_default_top_k_is_5():
    ...
    registry = MagicMock()
    BM25BaselinePipeline(bm25=bm25, registry=registry).run("q")
    bm25.search.assert_called_once_with("q", k=5)


def test_passes_through_top_k_to_bm25():
    bm25 = MagicMock()
    bm25.search.return_value = []
    registry = MagicMock()
    BM25BaselinePipeline(bm25=bm25, registry=registry, top_k=12).run("q")
    bm25.search.assert_called_once_with("q", k=12)
...
def test_template_answer_falls_back_to_doc_id_when_no_title():
    bm25 = MagicMock()
    bm25.search.return_value = [_hit("xx_yy", "1", "نص")]
    registry = MagicMock()
    registry.get_doc.return_value = None  # unknown doc
    pipeline = BM25BaselinePipeline(bm25=bm25, registry=registry)
    out = pipeline.run("سؤال")

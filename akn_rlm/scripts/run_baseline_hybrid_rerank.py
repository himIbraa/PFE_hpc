"""Run AlgerianLegalBench v3.0 against the hybrid + rerank baseline.

Reuses the B3 RRF(BM25, Dense) candidate pool, then applies the
cross-encoder reranker (``akn_rlm.reranker.rerank`` — defaults to the
model named in :data:`akn_rlm.config.RERANKER_MODEL`) to choose the
top-K=5 articles. No LLM / KG / SPLADE / ColBERT machinery — only the
BM25 index, the dense FAISS index, the ArticleRegistry, and the
cross-encoder reranker model are loaded.

Output layout matches ``run_baseline_hybrid.py`` so
...
Usage:
    ...
    & $py scripts\\run_baseline_hybrid_rerank.py --stratified 2 --run-id baseline_hybrid_rerank_smoke

    # Full 244-question run
    & $py scripts\\run_baseline_hybrid_rerank.py --run-id baseline_hybrid_rerank
"""
...
from akn_rlm.baselines.hybrid_rerank_pipeline import (  # noqa: E402
    DEFAULT_K_EACH,
...
log = logging.getLogger("run_baseline_hybrid_rerank")


def _ensure_reranker_loadable() -> None:
    """Eagerly load the cross-encoder so we fail loud (not silently degrade)."""
    from akn_rlm import reranker as reranker_mod
    log.info("Loading cross-encoder reranker (%s) …", RERANKER_MODEL)
    model = reranker_mod._get_model(RERANKER_MODEL)
    if model is None:
        raise RuntimeError(
            f"Cross-encoder reranker {RERANKER_MODEL!r} failed to load. "
            "Check the sentence-transformers install and model availability "
...
def _run(
    ...
    *,
    ...
    log.info("Running %d questions through hybrid+rerank baseline …", len(questions))
    ...
    report_path.write_text(
        format_report(strata, title=f"Hybrid+Rerank baseline  {run_id}"),
        encoding="utf-8",
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run AlgerianLegalBench v3.0 against the hybrid + cross-encoder rerank baseline"
    )
    ...
    parser.add_argument("--top-k", type=int, default=DEFAULT_TOP_K,
                        help=f"Top-K reranked hits to cite (default: {DEFAULT_TOP_K})")
    ...
    parser.add_argument("--rerank-pool-size", type=int, default=DEFAULT_RERANK_POOL_SIZE,
                        help=f"Top-N fused candidates fed to the reranker (default: {DEFAULT_RERANK_POOL_SIZE})")
    args = parser.parse_args()

    run_id = args.run_id or time.strftime("baseline_hybrid_rerank_%Y%m%d_%H%M%S")
    output_dir = Path(args.output_dir)
    benchmark_path = Path(args.benchmark) if args.benchmark else get_benchmark_path()

    log.info("=" * 64)
    log.info("Hybrid+Rerank baseline  run_id=%s", run_id)
    log.info("Benchmark : %s", benchmark_path)
    log.info("Output    : %s/%s/", output_dir, run_id)
    log.info("Top-K     : %d   K-each: %d   Pool: %d   Model: %s",
             args.top_k, args.k_each, args.rerank_pool_size, RERANKER_MODEL)
    log.info("=" * 64)
    ...
    # Fail loud BEFORE doing any work if the cross-encoder isn't available.
    _ensure_reranker_loadable()
    ...
    strata = stratify(results)
    print_report(strata, title=f"Hybrid+Rerank baseline  {run_id}")

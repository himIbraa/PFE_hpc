"""Run AlgerianLegalBench v3.0 conceptual_definitional slice through
the Phase-2 conceptual_definitional handler (R4).

Loads:
  - article registry (corpus parse, ~3 s)
  - BM25 index
  - dense FAISS+e5-small index
  - doc-router (R1)
  - rdflib KG (~26 s parse) — for the kg_entity_lookup channel
  - LLM pool — for paraphraser, ADU extraction, summariser
...
Usage:
    ...
    # Smoke (full 12-question conceptual_definitional slice — that's the gate)
    & $py scripts\\run_handler_conceptual_definitional.py \\
          --run-id rlm_conceptual_definitional_smoke

    # Stratified bucket if you want a wider read across all types
    & $py scripts\\run_handler_conceptual_definitional.py \\
          --query-types conceptual_definitional --stratified 2 \\
          --run-id rlm_conceptual_strat2
"""
...
from akn_rlm.rlm.handlers import build_conceptual_definitional_handler  # noqa: E402
from akn_rlm.rlm.handlers.conceptual_definitional import (  # noqa: E402
...
log = logging.getLogger("run_handler_conceptual_definitional")
...
def _run(
    ...
    *,
    ...
    log.info(
        "Running %d questions through conceptual_definitional handler …",
        len(questions),
    )
    ...
    report_path.write_text(
        format_report(strata, title=f"RLM conceptual_definitional handler  {run_id}"),
        encoding="utf-8",
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Run AlgerianLegalBench v3.0 against the Phase-2 "
            "conceptual_definitional handler"
        )
    )
    ...
    parser.add_argument(
        ...
        # Default to conceptual_definitional only — the gate the handler is designed to clear.
        default=["conceptual_definitional"],
        help="Only evaluate these query types (default: conceptual_definitional)",
    )
    ...
    parser.add_argument("--paraphrase-count", type=int, default=DEFAULT_PARAPHRASE_COUNT)
    parser.add_argument("--adu-extract-top-n", type=int, default=DEFAULT_ADU_EXTRACT_TOP_N)
    parser.add_argument("--kg-limit",         type=int, default=DEFAULT_KG_LIMIT)
    ...
    parser.add_argument("--no-kg", action="store_true",
                        help="Skip KG load — handler will rely on paraphrase fallback only")
    args = parser.parse_args()

    run_id = args.run_id or time.strftime("rlm_conceptual_definitional_%Y%m%d_%H%M%S")
    output_dir = Path(args.output_dir)
    benchmark_path = Path(args.benchmark) if args.benchmark else get_benchmark_path()

    log.info("=" * 64)
    log.info("RLM conceptual_definitional handler  run_id=%s", run_id)
    ...
    if args.no_kg:
        log.info("--no-kg passed; KG load skipped (handler will use paraphrase fallback).")
        kg = None
    else:
    ...
    handler = build_conceptual_definitional_handler(
        ...
        adu_extract_top_n=args.adu_extract_top_n,
    ...
    strata = stratify(results)
    print_report(strata, title=f"RLM conceptual_definitional handler  {run_id}")

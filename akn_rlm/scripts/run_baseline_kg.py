"""Run AlgerianLegalBench v3.0 against the KG-only baseline (B5).

Deterministic SPARQL retrieval over ``data/kg/algerian_legal_kg.ttl`` ‑ no
LLM, no BM25, no Dense, no reranker. For each significant token in the
query the pipeline issues a single UNION SPARQL searching paragraph,
provision and condition text for substring matches and aggregates the
matching article URIs by token coverage. The top-K (default 5) articles
are emitted as the same Arabic template answer used by B1-B4.

Optionally, a hybrid (B3) fallback can be wired in via ``--with-fallback``;
on ``no_kg_hits`` the pipeline delegates to the fallback and the telemetry
baseline is rewritten to ``"kg+fallback"``.
...
Usage:
    ...
    & $py scripts\\run_baseline_kg.py --stratified 2 --run-id baseline_kg_smoke

    # Full 244-question run
    & $py scripts\\run_baseline_kg.py --run-id baseline_kg

    # KG with hybrid fallback for no-hit queries
    & $py scripts\\run_baseline_kg.py --stratified 2 --with-fallback --run-id baseline_kg_fb_smoke
"""
...
from akn_rlm.baselines.kg_pipeline import (  # noqa: E402
    DEFAULT_TOP_K,
    build_kg_pipeline,
)
from akn_rlm.baselines.hybrid_pipeline import (  # noqa: E402
    DEFAULT_K_EACH as HYBRID_K_EACH,
    DEFAULT_TOP_K as HYBRID_TOP_K,
...
log = logging.getLogger("run_baseline_kg")
...
def _run(
    ...
    *,
    ...
    log.info("Running %d questions through KG baseline …", len(questions))
    ...
    report_path.write_text(
        format_report(strata, title=f"KG baseline  {run_id}"),
        encoding="utf-8",
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run AlgerianLegalBench v3.0 against the KG-only baseline"
    )
    ...
    parser.add_argument("--top-k", type=int, default=DEFAULT_TOP_K,
                        help=f"Top-K articles to cite (default: {DEFAULT_TOP_K})")
    parser.add_argument("--with-fallback", action="store_true",
                        help="On no_kg_hits, fall back to the hybrid (RRF BM25+Dense) "
                             "baseline. Telemetry baseline becomes 'kg+fallback'.")
    args = parser.parse_args()

    run_id = args.run_id or time.strftime("baseline_kg_%Y%m%d_%H%M%S")
    output_dir = Path(args.output_dir)
    benchmark_path = Path(args.benchmark) if args.benchmark else get_benchmark_path()

    log.info("=" * 64)
    log.info("KG baseline  run_id=%s", run_id)
    log.info("Benchmark : %s", benchmark_path)
    log.info("Output    : %s/%s/", output_dir, run_id)
    log.info("Top-K     : %d   Fallback: %s", args.top_k, args.with_fallback)
    log.info("=" * 64)
    ...
    if args.with_fallback:
        if not BM25_INDEX_PATH.exists():
            raise FileNotFoundError(
                f"BM25 index missing at {BM25_INDEX_PATH} — "
        ...
        log.info("Loading BM25 + Dense for fallback …")
        bm25 = BM25Index.load(BM25_INDEX_PATH)
        dense = DenseIndex.load(DENSE_FAISS_PATH, DENSE_META_PATH)
        fallback = build_hybrid_pipeline(
            bm25=bm25, dense=dense, registry=registry,
            top_k=HYBRID_TOP_K, k_each=HYBRID_K_EACH,
        )

    pipeline = build_kg_pipeline(
        kg=kg, registry=registry, top_k=args.top_k, fallback=fallback,
    )
    ...
    strata = stratify(results)
    print_report(strata, title=f"KG baseline  {run_id}")

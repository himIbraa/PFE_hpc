"""Run AlgerianLegalBench v3.0 against the BM25-only baseline pipeline.

This skips all LLM / KG / dense / SPLADE machinery — only the BM25 index and
ArticleRegistry are loaded. Output layout matches ``run_benchmark.py`` so
...
Usage:
    ...
    # Stratified smoke (16-q sample across all 8 query types — a few seconds)
    & $py scripts\\run_baseline_bm25.py --stratified 2 --run-id baseline_bm25_smoke

    # Full 244-question run (still seconds — no LLM calls)
    & $py scripts\\run_baseline_bm25.py --run-id baseline_bm25
"""
...
from akn_rlm.baselines.bm25_pipeline import (  # noqa: E402
    DEFAULT_TOP_K,
    build_bm25_pipeline,
)
...
log = logging.getLogger("run_baseline_bm25")
...
def _run(
    ...
    *,
    ...
    log.info("Running %d questions through BM25 baseline …", len(questions))
    ...
    report_path.write_text(
        format_report(strata, title=f"BM25 baseline  {run_id}"),
        encoding="utf-8",
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run AlgerianLegalBench v3.0 against the BM25-only baseline"
    )
    ...
    parser.add_argument("--top-k", type=int, default=DEFAULT_TOP_K,
                        help=f"BM25 top-K to cite (default: {DEFAULT_TOP_K})")
    args = parser.parse_args()

    run_id = args.run_id or time.strftime("baseline_bm25_%Y%m%d_%H%M%S")
    output_dir = Path(args.output_dir)
    benchmark_path = Path(args.benchmark) if args.benchmark else get_benchmark_path()

    log.info("=" * 64)
    log.info("BM25 baseline  run_id=%s", run_id)
    ...
    pipeline = build_bm25_pipeline(bm25=bm25, registry=registry, top_k=args.top_k)
    ...
    strata = stratify(results)
    print_report(strata, title=f"BM25 baseline  {run_id}")

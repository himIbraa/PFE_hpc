"""Run AlgerianLegalBench v3.0 against the dense-only baseline pipeline.

This skips all LLM / KG / BM25 / SPLADE machinery — only the FAISS dense
index (multilingual-e5-small) and ArticleRegistry are loaded. Output layout
matches ``run_benchmark.py`` so ``scripts/compare_baselines.py`` can read it
directly:
...
Usage:
    ...
    & $py scripts\\run_baseline_dense.py --stratified 2 --run-id baseline_dense_smoke

    # Full 244-question run
    & $py scripts\\run_baseline_dense.py --run-id baseline_dense
"""
...
from akn_rlm.baselines.dense_pipeline import (  # noqa: E402
    DEFAULT_TOP_K,
...
log = logging.getLogger("run_baseline_dense")
...
def _run(
    ...
    *,
    ...
    log.info("Running %d questions through dense baseline …", len(questions))
    ...
    report_path.write_text(
        format_report(strata, title=f"Dense baseline  {run_id}"),
        encoding="utf-8",
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run AlgerianLegalBench v3.0 against the dense-only baseline"
    )
    ...
    parser.add_argument("--top-k", type=int, default=DEFAULT_TOP_K,
                        help=f"Dense top-K to cite (default: {DEFAULT_TOP_K})")
    args = parser.parse_args()

    run_id = args.run_id or time.strftime("baseline_dense_%Y%m%d_%H%M%S")
    output_dir = Path(args.output_dir)
    benchmark_path = Path(args.benchmark) if args.benchmark else get_benchmark_path()

    log.info("=" * 64)
    log.info("Dense baseline  run_id=%s", run_id)
    ...
    pipeline = build_dense_pipeline(dense=dense, registry=registry, top_k=args.top_k)
    ...
    strata = stratify(results)
    print_report(strata, title=f"Dense baseline  {run_id}")

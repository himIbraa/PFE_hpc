"""Run AlgerianLegalBench v3.0 rule_application slice through the
Phase-2 rule_application handler.

This is the smoke runner for R6.1. It loads:
...
Usage:
    ...
    # Default smoke — full rule_application slice (≈ 66 questions)
    & $py scripts\\run_handler_rule_application.py \\
          --run-id rlm_rule_application_smoke

    # Stratified-N sample
    & $py scripts\\run_handler_rule_application.py \\
          --query-types rule_application \\
          --stratified 5 \\
          --run-id rlm_rule_application_strat5
"""
...
from akn_rlm.rlm.handlers import build_rule_application_handler  # noqa: E402
...
log = logging.getLogger("run_handler_rule_application")
...
def _run(
    ...
    *,
    ...
    log.info("Running %d questions through rule_application handler …", len(questions))
    ...
    report_path.write_text(
        format_report(strata, title=f"RLM rule_application handler  {run_id}"),
        encoding="utf-8",
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run AlgerianLegalBench v3.0 against the Phase-2 rule_application handler"
    )
    ...
    parser.add_argument(
        ...
        default=["rule_application"],
        help="Only evaluate these query types (default: rule_application)",
    )
    ...
    run_id = args.run_id or time.strftime("rlm_rule_application_%Y%m%d_%H%M%S")
    output_dir = Path(args.output_dir)
    benchmark_path = Path(args.benchmark) if args.benchmark else get_benchmark_path()

    log.info("=" * 64)
    log.info("RLM rule_application handler  run_id=%s", run_id)
    ...
    handler = build_rule_application_handler(
    ...
    strata = stratify(results)
    print_report(strata, title=f"RLM rule_application handler  {run_id}")
